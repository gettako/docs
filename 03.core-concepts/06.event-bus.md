...
> [!TIP]
> Use the RPC Bus when Plugin A needs data from Plugin B, but you don't want Plugin A to hard-import Plugin B's package.

### Registering an Endpoint (Server)

A plugin can expose a route and handle incoming requests:

```go [server.go]
// Expose "user.get" with a maximum 5-second processing limit
ctx.Events().(contracts.RPCBus).Route("user.get").
...
### Calling an Endpoint (Client)

Another plugin can call this endpoint using a fluent API with built-in retry mechanics:

```go [client.go]
// Call the "user.get" endpoint with retry support
res, err := ctx.Events().(contracts.RPCBus).Call("user.get").
    WithPayload(123).
    Retry(3). // Automatically retry up to 3 times if it fails or times out
    Await()
...
> [!WARNING]
> The RPC Bus includes **Anti-Cyclic Protection**. If Plugin A calls B, B calls C, and C calls A, the framework detects the infinite loop (maximum depth: 10) and safely aborts the request instead of crashing the thread.
