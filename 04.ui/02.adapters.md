...
Tako provides an official Bubble Tea adapter as a **separate Go submodule** at `github.com/takoterm/tako/pkg/adapter/bubbletea`. The core framework has **zero UI library dependencies** — installing `github.com/takoterm/tako` does not pull in Bubble Tea, Lipgloss, or Bubbles. The `pkg/adapter.BaseAdapter` helper is also available for building custom adapters. **Using them is optional**; you can implement `contracts.UIRenderer` directly for any other library.
