...
```mermaid
flowchart TD
    A["app.Boot called"] --> B["Create directories (config/, data/, .tako/)"]
    B --> C["Start Logger → data/logs/tako.log"]
    C --> D["Start KV Store → data/kv/kv.db"]
    D --> E{"Dev mode?\ngo run or\nTAKO_PROFILER_ENABLED=true"}
    E -- Yes --> F["Start Profiler + Debug Server"]
    E -- No --> G["Load Config → config/*.yaml|json"]
    F --> G
