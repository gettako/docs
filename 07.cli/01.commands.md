...
| `demo` | Runs an interactive UI component playground |
...
| `config:dump` | Dumps the resolved application configuration |
| `event:list` | Lists all registered event listeners |
| `hook:list` | Lists all registered framework hooks |
...
You can optionally implement `Signature() string` or `Configure(b *cli.CommandBuilder)` to define options.
...
func (c *ExportCommand) Signature() string {
    return `export 
            {--format=json : Output format: json, csv, tsv}
            {--o|output=export.json : Output file path}`
}

func (c *ExportCommand) Execute(ctx *cli.Context, args []string) error {
    c.format = ctx.OptionString("format")
    c.output = ctx.OptionString("output")
...
```bash
myapp export --format csv -o data.csv
```
