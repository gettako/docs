...
1. **UI Agnostic** — Tako's core has **zero UI library dependencies**. It abstracts rendering behind a `UIRenderer` interface. You choose the library: an official **Bubble Tea** adapter is available as an optional submodule (`github.com/takoterm/tako/pkg/adapter/bubbletea`), but you can bring `tview`, `tcell`, `termbox`, or anything you want. Installing `github.com/takoterm/tako` does not pull in any UI library.
